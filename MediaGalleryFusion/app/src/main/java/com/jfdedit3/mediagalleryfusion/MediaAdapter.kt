...
import com.jfdedit3.mediagalleryfusion.databinding.ItemMediaBinding
...
class MediaAdapter(
    ...
    inner class MediaViewHolder(private val binding: ItemMediaBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bind(item: MediaItemModel) {
            ...
            binding.infoLine.text = buildString {
                append(item.bucketName)
                if (item.durationMs > 0) append(" • ").append(item.durationMs / 1000).append("s")
                if (state.isFavorite(item.id)) append(" • ★")
            }
            ...
            if (item.type == MediaType.AUDIO) {
                ...
                binding.thumbnail.scaleType = if (settings.viewMode == ViewMode.LIST) ImageView.ScaleType.CENTER_CROP else ImageView.ScaleType.CENTER_CROP
            ...
            binding.favoriteBadge.visibility = if (state.isFavorite(item.id)) View.VISIBLE else View.GONE
